...
def load_frame_processor_module(frame_processor: str) -> Any:
    try:
        frame_processor_module = importlib.import_module(f'roop.roop.processors.frame.{frame_processor}')
...
def multi_process_frame(source_path: str, temp_frame_paths: List[str],
    ...
    with ThreadPoolExecutor(max_workers=roop_globals.execution_threads) as executor:
        futures = []
        queue = create_queue(temp_frame_paths)
        queue_per_future = max(len(temp_frame_paths) // roop_globals.execution_threads, 1)
...
def update_progress(progress: Any = None) -> None:
    ...
    progress.set_postfix({
        ...
        'execution_providers': roop_globals.execution_providers,
        'execution_threads': roop_globals.execution_threads
    })
    progress.refresh()
    progress.update(1)
