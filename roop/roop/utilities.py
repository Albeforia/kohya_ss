...
def run_ffmpeg(args: List[str]) -> bool:
    commands = ['ffmpeg', '-hide_banner', '-loglevel', roop_globals.log_level]
...
def extract_frames(target_path: str, fps: float = 30) -> bool:
    ...
    temp_frame_quality = roop_globals.temp_frame_quality * 31 // 100
    return run_ffmpeg(
        ['-hwaccel', 'auto', '-i', target_path, '-q:v', str(temp_frame_quality), '-pix_fmt', 'rgb24', '-vf',
         'fps=' + str(fps), os.path.join(temp_directory_path, '%04d.' + roop_globals.temp_frame_format)])


def create_video(target_path: str, fps: float = 30) -> bool:
    temp_output_path = get_temp_output_path(target_path)
    temp_directory_path = get_temp_directory_path(target_path)
    output_video_quality = (roop_globals.output_video_quality + 1) * 51 // 100
    commands = ['-hwaccel', 'auto', '-r', str(fps), '-i',
                os.path.join(temp_directory_path, '%04d.' + roop_globals.temp_frame_format), '-c:v',
                roop_globals.output_video_encoder]
    if roop_globals.output_video_encoder in ['libx264', 'libx265', 'libvpx']:
        commands.extend(['-crf', str(output_video_quality)])
    if roop_globals.output_video_encoder in ['h264_nvenc', 'hevc_nvenc']:
...
def get_temp_frame_paths(target_path: str) -> List[str]:
    temp_directory_path = get_temp_directory_path(target_path)
    return glob.glob((os.path.join(glob.escape(temp_directory_path), '*.' + roop_globals.temp_frame_format)))
...
def clean_temp(target_path: str) -> None:
    ...
    if not roop_globals.keep_frames and os.path.isdir(temp_directory_path):
