...
class PatchMatcher:
    def __init__(self, height, width, channel, patch_size, threads_per_block=8, num_iter=6, gpu_id=0,
                 guide_weight=100.0):
        self.height = height
        self.width = width
...
class PyramidPatchMatcher:
    def __init__(self, image_height, image_width, channel, patch_size, threads_per_block=8, num_iter=6, gpu_id=0,
                 guide_weight=100.0):
...
class VideoWithOperator:
    def __init__(self, frames_guide, frames_style, patch_size=21, threads_per_block=8, num_iter=6, gpu_id=0,
                 guide_weight=100.0):
...
class FastBlendingAlgorithm:
    def __init__(self, data, message=""):
        ...
        level = 1
        while (1 << level) <= n:
            for i in tqdm(range((1 << level) - 1, n, 1 << level),
                          desc=f"Preparing remapping table ({message}, level-{level})"):
...
def read_frames(frames_dir, sorter=None):
...
def smooth_video_frames(
    ...
    # input
    frames_guide = read_frames(video_guide, sorter)
    frames_style = read_frames(video_style, sorter)
    frames_guide, frames_style = align_frames(frames_guide, frames_style)
    # process
    smoother = PySynthSmoother(
        mode=mode,
    ...
    frames_output = smoother.smooth(frames_guide, frames_style)
    save_frames(frames_output, output_path)
