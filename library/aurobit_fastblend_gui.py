...
class PatchMatcher:
    ...
    def apply_nnf_to_image(self, nnf, source):
        ...
        target = cp.zeros((batch_size, self.height + self.pad_size * 2, self.width + self.pad_size * 2, self.channel),
                          dtype=cp.float32)
        remapping_kernel(
    ...
    def get_pairwise_patch_error(self, source, nnf):
        ...
        pairwise_patch_error_kernel(
            self.grid + (batch_size,),
            self.block,
            (self.height, self.width, self.channel, self.patch_size, self.pad_size, source_a, nnf_a, source_b, nnf_b,
             error)
        )
...
class FastModeRunner:
    ...
    def run(self, frames_guide, frames_style, batch_size, window_size, ebsynth_config, save_path=None):
        ...
        # left part
        table_l = table_manager.build_remapping_table(frames_guide, frames_style, patch_match_engine, batch_size,
                                                      desc="Fast Mode Step 1/4")
        table_l = table_manager.remapping_table_to_blending_table(table_l)
        table_l = table_manager.process_window_sum(frames_guide, table_l, patch_match_engine, window_size, batch_size,
                                                   desc="Fast Mode Step 2/4")
        # right part
        table_r = table_manager.build_remapping_table(frames_guide[::-1], frames_style[::-1], patch_match_engine,
                                                      batch_size, desc="Fast Mode Step 3/4")
        table_r = table_manager.remapping_table_to_blending_table(table_r)
        table_r = table_manager.process_window_sum(frames_guide[::-1], table_r, patch_match_engine, window_size,
                                                   batch_size, desc="Fast Mode Step 4/4")[::-1]
        # merge
        frames = []
...
class AccurateModeRunner:
    ...
    def run(self, frames_guide, frames_style, batch_size, window_size, ebsynth_config, desc="Accurate Mode",
            save_path=None):
        patch_match_engine = PyramidPatchMatcher(
            image_height=frames_style[0].shape[0],
            image_width=frames_style[0].shape[1],
...
class InterpolationModeRunner:
    ...
    def run(self, frames_guide, frames_style, index_style, batch_size, ebsynth_config, desc="Interpolate",
            save_path=None):
        patch_match_engine = PyramidPatchMatcher(
            image_height=frames_style[0].shape[0],
            image_width=frames_style[0].shape[1],
...
def interpolate_video(
    ...
    InterpolationModeRunner().run(frames_guide, frames_style, index_style, batch_size=batch_size,
                                  ebsynth_config=ebsynth_config, save_path=output_frames_path)
...
Given a guide video and a style video, this algorithm will make the style video fluent according to the motion features of the guide video. Click [here](https://github.com/Artiprocher/sd-webui-fastblend/assets/35051019/208d902d-6aba-48d7-b7d5-cd120ebd306d) to see the example. Note that this extension doesn't support long videos. Please use short videos (e.g., several seconds). The algorithm is mainly designed for 512*512 resolution. Please use a larger `Minimum patch size` for higher resolution.
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    minimum_patch_size = gr.Slider(label="Minimum patch size (odd number)", value=5, minimum=5,
                                                   maximum=99, step=2, interactive=True)
...
Given a guide video and some rendered keyframes, this algorithm will render the remaining frames.
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    with gr.Row():
                        detected_frames = gr.Textbox(label="Detected frames",
                                                     value="Please input the directory of guide video and rendered frames",
                                                     lines=9, max_lines=9, interactive=False)
                    video_guide_folder_.change(detect_frames, inputs=[video_guide_folder_, rendered_keyframes_],
                                               outputs=detected_frames)
                    rendered_keyframes_.change(detect_frames, inputs=[video_guide_folder_, rendered_keyframes_],
                                               outputs=detected_frames)
                with gr.Column():
                    output_path_ = gr.Textbox(label="Output directory", value="",
            ...
            with gr.Row():
                with gr.Column():
                    gr.Markdown("# Settings")
                    batch_size_ = gr.Slider(label="Batch size", value=8, minimum=1, maximum=128, step=1,
                                            interactive=True)
                    gr.Markdown("## Advanced Settings")
                    minimum_patch_size_ = gr.Slider(label="Minimum patch size (odd number)", value=15, minimum=5,
                                                    maximum=99, step=2, interactive=True)
                    num_iter_ = gr.Slider(label="Number of iterations", value=5, minimum=1, maximum=10, step=1,
                                          interactive=True)
                    guide_weight_ = gr.Slider(label="Guide weight", value=10.0, minimum=0.0, maximum=100.0, step=0.1,
                                              interactive=True)
                    initialize_ = gr.Radio(["identity", "random"], label="NNF initialization", value="identity",
                                           interactive=True)
                with gr.Column():
                    gr.Markdown("""
# Reference
