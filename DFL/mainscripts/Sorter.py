...
from DFL.DFLIMG import *
from DFL.core import mathlib, pathex
from DFL.core.cv2ex import *
from DFL.core.imagelib import estimate_sharpness
from DFL.core.interact import interact as io
from DFL.core.joblib import Subprocessor
from DFL.facelib import LandmarksProcessor
...
class BlurEstimatorSubprocessor(Subprocessor):
    ...
    # override
    def __init__(self, input_data, estimate_motion_blur=False, sharpness_threshold=0):
    ...
    def on_result(self, host_dict, data, result):
        if result[1] <= self.sharpness_threshold:
...
def sort_by_blur(input_path, drop_threshold):
    io.log_info("Sorting by blur...")

    img_list = [(filename, []) for filename in pathex.get_image_paths(input_path)]
    img_list, trash_img_list = BlurEstimatorSubprocessor(img_list, sharpness_threshold=drop_threshold).run()
...
def sort_by_hist_dissim(input_path):
    ...
    for filepath in io.progress_bar_generator(pathex.get_image_paths(input_path), "Loading"):
        ...
        img_list.append(
            [str(filepath), cv2.calcHist([cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)], [0], None, [256], [0, 256]), 0])
...
def sort_by_brightness(input_path):
    ...
    img_list = [[x, np.mean(cv2.cvtColor(cv2_imread(x), cv2.COLOR_BGR2HSV)[..., 2].flatten())] for x in
                io.progress_bar_generator(pathex.get_image_paths(input_path), "Loading")]
    io.log_info("Sorting...")
    img_list = sorted(img_list, key=operator.itemgetter(1), reverse=True)
    return img_list, []
...
def sort_by_hue(input_path):
    ...
    img_list = [[x, np.mean(cv2.cvtColor(cv2_imread(x), cv2.COLOR_BGR2HSV)[..., 0].flatten())] for x in
                io.progress_bar_generator(pathex.get_image_paths(input_path), "Loading")]
    io.log_info("Sorting...")
    img_list = sorted(img_list, key=operator.itemgetter(1), reverse=True)
    return img_list, []
...
class FinalHistDissimSubprocessor(Subprocessor):
    ...
    def __init__(self, pitch_yaw_sample_list):
        ...
        self.pitch_yaw_sample_list_idxs = [i for i in range(self.pitch_yaw_sample_list_len) if
                                           self.pitch_yaw_sample_list[i] is not None]
...
def main():
    # input_path, sort_by_method=None
