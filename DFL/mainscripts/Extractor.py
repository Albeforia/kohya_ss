    ...
    class Cli(Subprocessor.Cli):
        ...
        def on_initialize(self, client_dict):
            ...
            if self.type == 'all' or self.type == 'rects-s3fd' or 'landmarks' in self.type:
                self.rects_extractor = S3FDExtractor(place_model_on_cpu=place_model_on_cpu)

            if self.type == 'all' or 'landmarks' in self.type:
                # for head type, extract "3D landmarks"
                self.landmarks_extractor = FANExtractor(landmarks_3D=self.face_type >= FaceType.HEAD,
        ...
        def process_data(self, data):
            ...
            if cached_filepath != filepath:
                ...
                if image is None:
                    self.log_err(f'[ERROR]Failed to open {filepath}, reason: cv2_imread() fail.')
                    return data
...
def main():
    ...
    manual_output_debug_fix = False
    manual_window_size = 1368
    max_faces_from_image = 1  # Only support 1 face now
    cpu_only = False
    force_gpu_idxs = None
    ...
    parser.add_argument('face_type', type=str)
